...
const UserDetails = () => {
    ...
    useApiReq('http://localhost:3001/users/roles', roles, setRoles)
    ...
    return (
        <div className={conteiner}>
            ...
            <div className={ContentsubTitleUsers}>
                <h1 className={subtileUsers}><AiOutlineArrowLeft onClick={() => navigate(-1) } />{` ${userLoaded.firstName} ${userLoaded.lastName}`}</h1>
            </div>
            {/* DETAILS */}
                ...
                <div className={detailCont}>
                    <div className="row no-gutters">
                        ...
                        <div className="col-lg-4 card-body p-4">
                            <p><strong className={headingsColor}>Account</strong></p>
                            <p className="text-muted mb-0">Set your account type &amp; details.</p>
                        </div>

                        {/* RIGHT */}
                        <form className={`col-lg-8 card-body p-4 text-start ${rightCont}`} onSubmit={submitHandler}>
                            <div className="row">
                                <div className="col">
                                    <div className="form-group">
                                        <label htmlFor="fname" className={headingsColor}>First name</label>
                                        <input
                                ...
                                <div className="col">
                                    <div className="form-group">
                                        <label htmlFor="lname" className={headingsColor}>Last name</label>
                                        <input
                                            name="lastName"
                                            type="text"
                                            className={`form-control ${inputProf}`}
                                ...
                                {inputErros.firstName ? <div className="text-end"><small className="text-danger">{inputErros.firstName}</small></div> : null}
                                {inputErros.lastName ? <div className="text-end"><small className="text-danger">{inputErros.lastName}</small></div> : null}
                            </div>

                            <div className="form-group">
                            <label htmlFor="fname" className={headingsColor}>Email</label>
                                <input 
                                    name="email"
                                    type="email"
                                    className={`form-control ${inputProf}`}
                                    ...
                                    onChange={handleChangeInput}    
                                />
                                {inputErros.email ? <div className="text-end"><small className="text-danger">{inputErros.email}</small></div> : null}
                            </div>

                            <div className="row">
                                <div className="col">
                                    <div className="form-group">
                                        <label htmlFor="phone" className={headingsColor}>Phone</label>
                                        <input
                                            name="phone"
                                            type="text"
                                            className={`form-control ${inputProf}`}
                                ...
                                <div className="col">
                                    <div className="form-group">
                                        <label htmlFor="role" className={headingsColor}>Role</label><br/>
                                        <select name="role" className="form-select" value={input.role} onChange={handleChangeInput}>
                                            {Object.keys(roles).length > 0 &&  input.role && roles.map(r => {
                                                const option = <option  key={r.id} value={r.id}>{r.name}</option>;
                                                if (r.name !== 'Super admin') return option;
                                            })}
                                        </select>
                                    </div>
                                </div>
                                {inputErros.phone ? <div className="text-end"><small className="text-danger">{inputErros.phone}</small></div> : null}
                            </div>
                            { 
                                false ?
                                <div className="text-end"><small className="text-danger">{false}</small></div>:
                                null
                             }
                             { 
                                userUpdareErr === "success" ?
                                <div className="text-end"><small className="text-success">User updated!!</small></div>:
                                null
                             }
                ...
                <table>
                    ...
                    <tbody>
                        <tr className={rowsUsers}>
                            <td><strong>Rock Concert</strong></td>
                            <td>email</td>
                            <td>role</td>
