...
const DashboardAdmin = ({ children }) => {
  ...
  let vendors = users.length
    ? users.filter((user) => user.Role.name == "Vendedor").length
    : "...";
  let customers = users.length
    ? users.filter((user) => user.Role.name == "User").length
    : "...";
    ...
    <div className={conteiner}>
      ...
      <div className={cardConteiner}>
        ...
        <CardAnalitics
          ...
          estadistics={new Intl.NumberFormat("en-US", {
            style: "percent",
          }).format(vendors / totUsers)}
          img={calen}
        />
        <CardAnalitics
